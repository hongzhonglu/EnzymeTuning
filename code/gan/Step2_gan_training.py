...
def main():
    ...
    path_parameters = 'gan_input/kcat_data_new_1000_50_score.h5'
    f = h5py.File (path_parameters , 'r')

    # load rxnlist(kcat_name)
    with open ('gan_input/parameter_sen_50.pkl' , 'rb') as file:
        parameter_names = pickle.load (file)

    # fetch training set eigenvalues
    path_stability = 'gan_input/pro_new_1000_50_score.csv'
    if not path_stability.endswith ('.csv'):
        raise ValueError ('Your data must be a .csv file')
    pro_data = pd.read_csv (path_stability).iloc[: , 0:2].values
    error = 1 - (pro_data[: , 0] - pro_data[: , 0].min ())/(pro_data[: , 0].max () - pro_data[: , 0].min ())
    corr = (pro_data[: , 1] - pro_data[: , 1].min ())/(pro_data[: , 1].max () - pro_data[: , 1].min ())
    w1 , w2 = 0.5 , 0.5  # weight
    stabilities_0 = w1*error + w2*corr
    ...
    for j in range (3): #10, 30, 50, test for 2 or 3
        ...
        # save everything
        exp_id = 'test' + str (j)
        savepath = f'result/gan_sen_sen_score_50/gan_input/{exp_id}'
        ...
        savepath = f'result/gan_sen_sen_score_50/gan_output/{exp_id}/'
        os.makedirs (savepath , exist_ok=True)
        ...
        # load new kcat
        kcat_new_name = f'result/gan_sen_sen_score_50/gan_output/test{j}/repeat_{repeats - 1}/490_r.npy'
        ...
        with Pool (processes=num_cpus) as pool:
            args = [(
                j , growthdata , eModel , model , kcat_final_tmp , proteomicsdata_Clim , proteomicsdata_Nlim , toy_data) for
        ...
        error_new = 1 - (toy_data['error'] - toy_data['error'].min ())/(toy_data['error'].max () - toy_data['error'].min ())
        corr_new = (toy_data['corr'] - toy_data['corr'].min ())/(toy_data['corr'].max () - toy_data['corr'].min ())
        w1 , w2 = 0.5 , 0.5  # weight
        stability_new = w1*error_new + w2*corr_new
        if j > 0:
        ...
        stability_sort = sorted (range (len (stability_all)) , key=lambda k: stability_all[k] , reverse=True)
        stabilities_0 = stability_all[stability_sort[0:1000]]
        savepath_sta = f'result/gan_sen_sen_score_50/gan_output/test{j}'
        ...
        path_parameters = f'result/gan_sen_sen_score_50/gan_input/test{j}/kcat_data_1000.h5'
