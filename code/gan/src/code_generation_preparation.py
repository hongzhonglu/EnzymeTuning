...
def anaerobicModel(model , model_cobra):
    ...
    mets = [ 's_3714[c]', 's_1198[c]', 's_1203[c]', 's_1207[c]', 's_1212[c]', 's_0529[c]' ]
    reaction = model_cobra.reactions.get_by_id ('r_4598')
    print (f"Metabolites in reaction 'r_4598': {reaction.metabolites}")

    for met in mets:
        if met in reaction.metabolites:
            coefficient = reaction.get_coefficient (met)
            reaction.add_metabolites ({ met: 0 - coefficient })
        else:
            print (f"Metabolite {met} not found in reaction 'r_4598'.")
    ...
    if len (set_strain_species):
        ...
        for i in range (len (model['rxns'])):
            if str (model['rxns'][i][0][0]).startswith ('r_0713'):
                id_0713.append (i)
        for reaction_id in id_0713:
            try:
                reaction = model_cobra.reactions.get_by_id (reaction_id) # Mithocondria
                reaction.lower_bound = 0
                print (f"Lower bound of reaction '{reaction_id}' has been set to 0.")
            except KeyError:
                print (f"Reaction ID '{reaction_id}' not found in the model.")

        reactions_to_modify = [
            ('r_0714', 'lower', 0),  # Cytoplasm
            ('r_0713_rvs', 'lower', 0),  # Mitochondria
            ('r_0714_rvs', 'lower', 0),  # Cytoplasm
            ('r_0487', 'upper', 0),  # Block glycerol dehydroginase
            ('r_0487_rvs', 'upper', 0),  # Block glycerol dehydroginase
            ('r_0472', 'upper', 0),  # Block alternative pathway
            ('r_0472_fwd', 'upper', 0)  # Block alternative pathway
        ]

        for reaction_id, bound_type, bound_value in reactions_to_modify:
            try:
                reaction = model_cobra.reactions.get_by_id (reaction_id)
                if bound_type=='lower':
                    reaction.lower_bound = bound_value
                elif bound_type=='upper':
                    reaction.upper_bound = bound_value
                print (f"{bound_type.capitalize ()} bound of reaction '{reaction_id}' has been set to {bound_value}.")
            except KeyError:
                print (f"Reaction ID '{reaction_id}' not found in the model.")
...
def getrSample(mu, sigma, lb, ub, step, method = 'Uniform'):
    ...
    for i in range(len(lb)):
        if lb[i] == ub[i] and lb[i] == 0:
            lb[i] = -5.5563 #10^-2/3600
            ub[i] = 3.4437 #10^7/3600
