...
def addEnzymesToRxn(rxn, kcat_dict, protIDs, rxn_index=None):
    ...
    prot_exchange_rxns = []
    for prot in prots:
        prot_met = Metabolite('{0}[{1}]'.format(prot,comp))
    ...
    # add enzymes into reaction
    e_rxn.add_metabolites({prot_met: enzyme_coefficients.get(prot_met.id.split('[')[0], 0) for prot_met in prot_mets})
...
def convertToEnzymeModel(model,kcats):
    ...
    for rxn in model.reactions:
        ...
        if len(complexes) >1:
            ...
            for i,complx in enumerate(complexes):
                ...
                e_rxn, prot_exchange_rxns = addEnzymesToRxn(rxn_new, kcats, complx,rxn_index=i+1)
        ...
        if len(complexes) == 1:
            ...
            prots = [item.strip() for item in complx.split('and')]
            kcat = kcats[(prots[0],rxn.id)]
            e_rxn, prot_exchange_rxns = addEnzymesToRxn(rxn, kcats, complx,rxn_index=1)
            converted_reaction_list.append(e_rxn)
            for prot_exchange_rxn in prot_exchange_rxns: protein_exchange_rxns[prot_exchange_rxn.id] = prot_exchange_rxn
    ...
    eModel.enzymes = set ([ '_'.join (exgrxn.split ('_')[ 1: ]) for exgrxn in protein_exchange_rxns.keys () ])
    print('Number of enzymes:', len(eModel.enzymes))

    return eModel

def turnover_sample(turnover, vsyn_x, vsyn_y, step):
    r = np.zeros((len(turnover), int(step)))

    for i in range(len(turnover)):
        turnovertmp = -np.log10(turnover[i])
        vsyn_x_tmp = -np.log10(vsyn_x[i])
        vsyn_y_tmp = -np.log10(vsyn_y[i])
        shape, loc, scale = lognorm.fit(turnovertmp, floc=0)
        lb, ub = np.minimum(vsyn_x_tmp, vsyn_y_tmp), np.maximum(vsyn_x_tmp, vsyn_y_tmp)
        a, b = (lb - loc) / scale, (ub - loc) / scale
        lognorm_samples = truncnorm.rvs(a, b, loc=loc, scale=scale, size=step)
        lognorm_samples[lognorm_samples < lb] = lb
        lognorm_samples[lognorm_samples > ub] = ub
        lognorm_samples = 10 ** (-lognorm_samples)
        r[i, :] = lognorm_samples
