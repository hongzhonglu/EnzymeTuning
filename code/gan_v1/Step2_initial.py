...
def main():
    ...
    proteo_df = proteomics_data.merge (scer_data[['Entry' , 'Gene Names']] ,
                                       left_on='Accession' ,
                                       right_on='Entry' ,
                                       how='left')
    ...
    proteo_df = proteo_df.merge (eModel_genes_series[['emodel_genes' , 'entry_to_gene']] ,
                                 left_on='Accession' ,
                                 right_on='entry_to_gene' ,
                                 how='inner')
    ...
    path_parameters = 'data/sylogin/kcat_data_1000.h5'
    ...
    with Pool (processes=num_cpus) as pool:
        args = [(
            j , growthdata_test , eModel , model , kcat_final , proteo_df , toy_data) for j in range (number_of_models)]
    ...
    toy_data.to_csv ('data/sylogin/pro_data1.csv' , index=False)
