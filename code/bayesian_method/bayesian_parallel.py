def main():
    ...
    from code_bayesian import abc_python_max
    from code_bayesian import parallel_task
    ...
    while D > rejectnum and D_100 > 0.5:
        output = output + 1
        nfound = len(os.listdir(r'result/bayesian_kcat'))
        print(nfound)
        if nfound > 0:
            tmp = pd.read_csv('result/bayesian_kcat/'+'kcat_genra' + str(output) + '.txt', delimiter=',',header= None)
            tmp = np.array(tmp)
            theta_100 = tmp[-1, :]
            kcat_100 = tmp[0:-2, :]
