def anaerobicModel(model):
    ...
    GAM = 58.1988
    strain = model['id'].replace(' specific model genereted from panYeast', '')
    ...
    met_index = model['mets'].find(mets)
    model['S'][met_index, model['rxns'].find('r_4598')] = 0
    # 3st change: Changes media to anaerobic
    for s in range(len(model['rxns'])):
        if model['rxn'][s] == 'r_1992':
            model['lb'][s] = 0
        elif model['rxn'][s] == 'r_1757':
            model['lb'][s] = -1000
        elif model['rxn'][s] == 'r_1994':
            model['lb'][s] = -1000
        elif model['rxn'][s] == 'r_1915':
            model['lb'][s] = -1000
        elif model['rxn'][s] == 'r_2106':
            model['lb'][s] = -1000
        elif model['rxn'][s] == 'r_2134':
            model['lb'][s] = -1000
        elif model['rxn'][s] == 'r_2137':
            model['lb'][s] = -1000
        elif model['rxn'][s] == 'r_2189':
            model['lb'][s] = -1000
    ...
    if len(set(species_intersect) & set(strain)):
        model.reactions.get_by_id('r_2090').lower_bound = -1000
    ...
    for s in range(len(model['rxns'])):
        ...
        if model['rxns'][s].startwith('r_0713'):
    ...
    # Block oxaloacetate-malate shuttle (not present in anaerobic conditions)
    for s in range(len(model['rxns'])):
        if 'r_0713' in model['rxns'][s] and 'rvs' in model['rxns'][s]:
            idx[s] = s
    ...
    for s in range(len(model['rxns'])):
        if 'r_0714' in model['rxns'][s] and 'rvs' in model['rxns'][s]:
            idx[s] = s
        if s in model['rxns'][idx]:
            model['ub'][s] = 0
            model['lb'][s] = 0
        if model['rxns'][s] == 'r_0714':
            ...
            # %Block 2-oxoglutarate + L-glutamine -> 2 L-glutamate (alternative pathway)
    for s in range(len(model['rxns'])):
        if model['rxns'][s].startwith('r_0472_'):
            idx[s] = s
        if s in model['rxns'][idx]:
            model['ub'][s] = 0
            model['lb'][s] = 0
